...
#region Unity Messages
	...
	private void InitOVRManager()
	{
		...
		Log("Unity v" + Application.unityVersion + ", " +
				"Oculus Utilities v" + OVRPlugin.wrapperVersion + ", " +
				"OVRPlugin v" + OVRPlugin.version + ", " +
				"SDK v" + OVRPlugin.nativeSDKVersion + ".");

		LogFormat("SystemHeadset {0}, API {1}", systemHeadsetType.ToString(), xrApi.ToString());

		if (xrApi == XrApi.OpenXR)
		{
			LogFormat("OpenXR instance 0x{0:X} session 0x{1:X}", xrInstance, xrSession);
		}
		...
		if (!staticMixedRealityCaptureInitialized)
		{
			...
			{
				Log("OVR: Mixed Reality mode enabled");
				...
				Log("OVR: CompositionMethod : " + compositionMethod);
			}
		}
#endif
		...
		LogFormat("Current display frequency {0}, available frequencies [{1}]",
	...
	private void Update()
	{
		...
		{
			Log("[OVRManager] OVRPlugin.shouldQuit detected");
#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN || OVR_ANDROID_MRC
		...
		{
			Log("The current MSAA level is " + QualitySettings.antiAliasing +
		...
		if (!_wasHmdPresent && isHmdPresent)
		{
			try
			{
				Log("[OVRManager] HMDAcquired event");
				if (HMDAcquired != null)
					HMDAcquired();
			}
			catch (Exception e)
		...
		if (_wasUserPresent && !isUserPresent)
		{
			try
			{
				Log("[OVRManager] HMDUnmounted event");
		...
		if (!_wasUserPresent && isUserPresent)
		{
			try
			{
				Log("[OVRManager] HMDMounted event");
				if (HMDMounted != null)
					HMDMounted();
			}
			catch (Exception e)
		...
		if (_hadVrFocus && !hasVrFocus)
		{
			try
			{
				Log("[OVRManager] VrFocusLost event");
				if (VrFocusLost != null)
					VrFocusLost();
			}
			catch (Exception e)
		...
		if (!_hadVrFocus && hasVrFocus)
		{
			try
			{
				Log("[OVRManager] VrFocusAcquired event");
		...
		if (_hadInputFocus && !hasInputFocus)
		{
			try
			{
				Log("[OVRManager] InputFocusLost event");
		...
		if (!_hadInputFocus && hasInputFocus)
		{
			try
			{
				Log("[OVRManager] InputFocusAcquired event");
		...
		else if (audioOutId != prevAudioOutId)
		{
			try
			{
				Log("[OVRManager] AudioOutChanged event");
		...
		else if (audioInId != prevAudioInId)
		{
			try
			{
				Log("[OVRManager] AudioInChanged event");
		...
		if (wasPositionTracked && !tracker.isPositionTracked)
		{
			try
			{
				Log("[OVRManager] TrackingLost event");
		...
		if (!wasPositionTracked && tracker.isPositionTracked)
		{
			try
			{
				Log("[OVRManager] TrackingAcquired event");
	...
	private static Camera FindMainCamera() {
		...
		if (result != null)
		{
			LogFormat("[OVRManager] mainCamera found for MRC: ", result.gameObject.name);
		}
		else
		{
			Log("[OVRManager] unable to find a vaild camera");
		}
	...
	private void OnApplicationPause(bool pause)
	{
		if (pause)
		{
			Log("[OVRManager] OnApplicationPause(true)");
		}
		else
		{
			Log("[OVRManager] OnApplicationPause(false)");
		}
	}
	...
	private void OnApplicationFocus(bool focus)
	{
		if (focus)
		{
			Log("[OVRManager] OnApplicationFocus(true)");
		}
		else
		{
			Log("[OVRManager] OnApplicationFocus(false)");
		}
	}

	private void OnApplicationQuit()
	{
		Log("[OVRManager] OnApplicationQuit");
	}
#endif
	...
	public static void StaticInitializeMixedRealityCapture(OVRMixedRealityCaptureConfiguration configuration)
	{
		if (!staticMixedRealityCaptureInitialized)
		{
			...
			Log(mediaInitialized ? "OVRPlugin.Media initialized" : "OVRPlugin.Media not initialized");
			if (mediaInitialized)
			{
				...
				if(audioConfig.sampleRate > 0)
				{
					...
					LogFormat("[MRC] SetMrcAudioSampleRate({0})", audioConfig.sampleRate);
				}

				OVRPlugin.Media.SetMrcInputVideoBufferType(OVRPlugin.Media.InputVideoBufferType.TextureHandle);
				LogFormat("[MRC] Active InputVideoBufferType:{0}", OVRPlugin.Media.GetMrcInputVideoBufferType());
				if (configuration.mrcActivationMode == MrcActivationMode.Automatic)
				{
					...
					LogFormat("[MRC] ActivateMode: Automatic");
				}
				else if (configuration.mrcActivationMode == MrcActivationMode.Disabled)
				{
					...
					LogFormat("[MRC] ActivateMode: Disabled");
				}
	...
	public static void StaticUpdateMixedRealityCapture(OVRMixedRealityCaptureConfiguration configuration, GameObject gameObject, TrackingOrigin trackingOrigin)
	{
		...
		if (configuration.enableMixedReality && !staticPrevEnableMixedRealityCapture)
		{
			...
			Log("MixedRealityCapture: activate");
		}

		if (!configuration.enableMixedReality && staticPrevEnableMixedRealityCapture)
		{
			Log("MixedRealityCapture: deactivate");
		}
